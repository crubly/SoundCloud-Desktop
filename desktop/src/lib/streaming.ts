...
import { STREAMING_BASE, STREAMING_PREMIUM_BASE } from './constants';
...
/**
 * SoundCloud-only build: SCD-storage (`storage.scnative.space`) вырезан.
 * Файлы качаются напрямую с SoundCloud через `/stream`/`/download` локального
 * сервера или sc_anon-fallback; отдельного хранилища нет.
 */
export function buildStorageUrls(_trackUrn: string): string[] {
  return [];
}
