...
// SoundCloud-only build: хостов больше нет, аппа живёт на локальном Rust-сервере
// (api) и прямом SoundCloud (api-v2). Вердикты зашиты в 'up'/'online' — probe-движок
// для scnative-хостов выключен, см. probe.ts.
...
export function isIncidentActive(): boolean {
  // Аварий у scnative-инфраструктуры больше нет — локальный сервер не «падает»,
  // SoundCloud либо отвечает, либо нет (это обрабатывают API/стриминг-пути).
  return false;
}
