...
import { YouTubeWall } from '../components/search/YouTubeWall';
...
export function Search() {
  ...
  const isUrl = isSoundCloudUrl(q) || isYouTubeUrl(q);
  const ytMode = source === 'yt';
  const query = ytMode ? debounced : isUrl ? '' : debounced;
  ...
  useEffect(() => {
    ...
    if (trimmed.length >= 2 && !isSoundCloudUrl(trimmed) && !isYouTubeUrl(trimmed)) {
      addQuery(trimmed);
    }
  }, [debounced, addQuery]);
