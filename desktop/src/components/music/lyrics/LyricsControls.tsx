...
const FullscreenDislikeButton = React.memo(({ track }: { track: Track }) => {
  ...
  const toggle = async () => {
    ...
    if (nowDisliked && isUrnLiked(track.urn)) {
      optimisticToggleLike(qc, track, false);
    }
    if (nowDisliked) purgeDislikedFromQueue(track.urn);
...
export const Controls = React.memo(({ track }: { track: Track }) => {
  ...
  return (
    <div className="flex items-center justify-center gap-2">
      ...
      {/* Дизлайк не показываем для треков из избранного. */}
      {!liked && <FullscreenDislikeButton track={track} />}
