...
/** Darken an rgb colour by `factor` (0..1, 1 = unchanged), with a brightness
 *  floor so a near-black cover can't turn the accent invisible. */
export function darkenRgb(color: [number, number, number], factor: number): string {
  const floor = 26;
  let dr = Math.round(color[0] * factor);
  let dg = Math.round(color[1] * factor);
  let db = Math.round(color[2] * factor);
  const lum = 0.299 * dr + 0.587 * dg + 0.114 * db;
  if (lum < floor) {
    const lift = Math.round(floor - lum);
    dr = Math.min(255, dr + lift);
    dg = Math.min(255, dg + lift);
    db = Math.min(255, db + lift);
  }
...
export function useArtworkColor(artworkUrl: string | null) {
  ...
  const [color, setColor] = useState<[number, number, number]>([255, 85, 0]);
