...
export function PlaybackCard() {
  ...
  const fullscreenHideTuning = useSettingsStore((s) => s.fullscreenHideTuning);
  const setFullscreenHideTuning = useSettingsStore((s) => s.setFullscreenHideTuning);
  ...
  return (
    <Card title={t('settings.playback')} icon={<Headphones size={17} />}>
      <div className="divide-y divide-white/[0.05]">
        ...
        <Row
          title={t('settings.fullscreenHideTuning')}
          desc={t('settings.fullscreenHideTuningDesc')}
        >
          <Toggle
            checked={fullscreenHideTuning}
            onChange={() => setFullscreenHideTuning(!fullscreenHideTuning)}
          />
        </Row>
      </div>
    </Card>
